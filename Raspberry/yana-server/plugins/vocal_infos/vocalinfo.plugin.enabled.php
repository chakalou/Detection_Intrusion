...
function vocalinfo_vocal_command(&$response,$actionUrl){
	...
	$response['commands'][] = array(
		'command'=>VOCAL_ENTITY_NAME.' lance détéction',
		'url'=>$actionUrl.'?action=vocalinfo_lance_detec_diag','confidence'=>'0.88'
		);
	$response['commands'][] = array(
		'command'=>VOCAL_ENTITY_NAME.' arrêt détéction',
		'url'=>$actionUrl.'?action=vocalinfo_arret_detec_diag','confidence'=>'0.88'
		);
...
function vocalinfo_action(){
	...
	switch($_['action']){
		...
		case 'vocalinfo_gpio_diag':
			...
			echo ($json=='[]'?'{}':$json);
		break;
		case 'vocalinfo_lance_detec_diag':
			$commands = array();
			system ("/home/pi/Projet/bin/lancerprog.sh & > /home/pi/Projet/log/log.log",$commands);
			$sentence='J\'ai lansset detection'.$commands[0];
			$response = array('responses'=>array(
								array('type'=>'talk','sentence'=>$sentence)
											)
						);
			...
			echo ($json=='[]'?'{}':$json);
		break;
		case 'vocalinfo_arret_detec_diag':
			$commands = array();
			system ("/home/pi/Projet/bin/arretprog.sh & > /home/pi/Projet/log/log.log",$commands);
			$sentence='J\'ai arrêté detection'.$commands[0];
			$response = array('responses'=>array(
								array('type'=>'talk','sentence'=>$sentence)
											)
						);
